...
def _battle_locked_result(action_code: str) -> dict:
    return {
        "success": False,
        "reason_code": "SPELL_ACTION_BATTLE_LOCKED",
        "reason_data": {
            "action": action_code
        }
    }
...
async def equip_spell(
    ...
    response_data = {
        ...
        "reason_data": dict(result.get("reason_data", {}), **{
            "spell_id": request.spell_id,
            "slot_type": result.get("spell_type", ""),
            "spell_type": result.get("spell_type", ""),
            "action": str(result.get("reason_data", {}).get("action", ""))
        })
    }
...
@router.post("/spell/star_up", response_model=dict)
async def star_up_spell(
    request: StarUpSpellRequest,
    ctx: GameContext = Depends(get_write_game_context),
    token_info: dict = Depends(get_token_info)
):
    """升星术法"""
    ...
    if ctx.lianli_system.is_battling:
        ...
        result = ctx.spell_system.star_up_spell(request.spell_id, ctx.inventory_system)
    ...
    if result["success"]:
        ...
        ctx.player_data.last_online_at = datetime.now(timezone.utc)
        await ctx.player_data.save()
        logger.info(f"[GAME] 术法升星成功 - account_id: {ctx.account.id} - spell_id: {request.spell_id} - new_star: {result.get('new_star', 0)}")
    ...
    response_data = {
        ...
        "reason_data": dict(result.get("reason_data", {}), **{
            "spell_id": request.spell_id,
            "new_star": result.get("new_star", 0),
            "action": str(result.get("reason_data", {}).get("action", "")),
        })
    }
    logger.info(f"[OUT] POST /game/spell/star_up - {json.dumps(response_data, ensure_ascii=False)} - 耗时：{time.time() - start_time:.4f}s")
    return response_data


@router.get("/spell/list", response_model=SpellListResponse)
async def get_spell_list(
    ...
    player_spells_payload = {
        ...
        for spell_id in SpellData.get_all_spells()
    }

    response_data = SpellListResponse(
        success=True,
        operation_id="",
        timestamp=time.time(),
        reason_code="SPELL_LIST_SUCCEEDED",
        reason_data={},
        player_spells=player_spells_payload,
        equipped_spells=ctx.spell_system.equipped_spells,
        spells_config=SpellData.get_spells_config()
    )
    logger.info(f"[OUT] GET /game/spell/list - {json.dumps(response_data.dict(), ensure_ascii=False)} - 耗时：{time.time() - start_time:.4f}s")
    return response_data
